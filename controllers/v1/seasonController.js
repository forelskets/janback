...
const seasonPayment = async (req, res, next) => {
  const userDetails = req.token;
  const obj = Object.assign({});
  if (req.file && req.file.path) obj.cardPhoto = req.file.path;
  else obj.cardNumber = req.body.photoCardNumber;
  obj.duration = JSON.parse(req.body.duration);
  obj.user = req.token._id;
  ...
  obj.validUpto = req.body.validUpto;
  obj.paymentType = req.body.paymentType;
  obj.source = req.body.source;
  obj.sourceCRSCode = req.body.sourceCRSCode;
